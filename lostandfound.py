...
class StudentDashboard:
    def login(self):
        ...
        if st.session_state.student_logged_in:
            ...
            # Note: Make sure you have uiu_campus.jpg in your project directory
            if os.path.exists("uiu_campus.jpg"):
                st.image("uiu_campus.jpg", use_container_width=True)
            else:
                st.info("Campus image not found. Please add 'uiu_campus.jpg' to your project directory.")
            ...
            if st.button("Login"):
                if name and student_id:
                    ...
                    st.rerun()  # Use st.rerun() instead of calling dashboard() directly
                else:
    ...
    def visualize_data():
        ...
        if os.path.exists(CSV_FILE):
            ...
            if chart_type == "Bar Chart":
                ...
                sns.barplot(x=chart_data.index, y=chart_data.values, palette="viridis", ax=ax)
                ...
                if not data_clean.empty:
                    chart_data = data_clean.groupby("Date").size()
                    fig = px.line(x=chart_data.index, y=chart_data.values, labels={"x": "Date", "y": "Count"}, title="Items Over Time")
                    st.plotly_chart(fig)
                else:
                    st.warning("No valid date data available for line graph.")

            elif chart_type == "Scatter Plot":
                data["Date"] = pd.to_datetime(data["Date"], errors="coerce")
                data_clean = data.dropna(subset=["Date"])
                if not data_clean.empty:
                    scatter_fig = px.scatter(data_clean, x="Date", y="Category", color="Type", title="Scatter Plot of Items")
                    st.plotly_chart(scatter_fig)
                else:
                    st.warning("No valid date data available for scatter plot.")

            elif chart_type == "Location Distribution":
                location_data = data[data["Type"] == "Lost"]["Location"].value_counts()
                if not location_data.empty:
                    fig, ax = plt.subplots(figsize=(10, 5))
                    sns.barplot(x=location_data.index, y=location_data.values, palette="viridis", ax=ax)
                    ...
                    st.info("No lost items data available for location distribution.")
